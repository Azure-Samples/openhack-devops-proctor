...
echo "11-Build and User-Profile API to AKS  (# bash ./build_deploy_user-profile.sh -s ./test_fetch_build -b Release -r $resourceGroupTeam -t 'user-java' -d $dnsURL -n ${teamName}${teamNumber} -g $registryName)"
bash ./build_deploy_user-profile.sh -s ./test_fetch_build -b Release -r $resourceGroupTeam -t 'user-java' -d $dnsURL -n ${teamName}${teamNumber} -g $registryName
