...
curl -X POST https://management.azure.com/subscriptions/$subscriptionId/resourceGroups/$resourceGroupName/providers/Microsoft.StreamAnalytics/streamingjobs/$streamAnalyticsJobName/start?api-version=2015-10-01 -H "Authorization: Bearer $token" -H "Content-type: application/json" -d '{"outputStartMode" : "JobStartTime"}' -v
