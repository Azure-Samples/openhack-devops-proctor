...
INSERT INTO leaderboard.dbo.Challenges (Id, TeamId, ChallengeDefinitionId, StartDateTime, EndDateTime, Score)
VALUES(NEWID(), @TeamId, @ChallengeDefinitionId, GETDATE()+0.5001, GETDATE()+1, 20);

select @ChallengeDefinitionId=Id from leaderboard.dbo.ChallengeDefinitions where Name = 'Challenge 3'

INSERT INTO leaderboard.dbo.Challenges (Id, TeamId, ChallengeDefinitionId, StartDateTime, EndDateTime, Score)
VALUES(NEWID(), @TeamId, @ChallengeDefinitionId, GETDATE()+1.001, GETDATE()+1.5, 30);

select @ChallengeDefinitionId=Id from leaderboard.dbo.ChallengeDefinitions where Name = 'Challenge - Medium 1'

INSERT INTO leaderboard.dbo.Challenges (Id, TeamId, ChallengeDefinitionId, StartDateTime, EndDateTime, Score)
VALUES(NEWID(), @TeamId, @ChallengeDefinitionId, GETDATE()+1.5001, GETDATE()+2, 60);
...
DECLARE @Start int;
DECLARE @End int;
DECLARE @I decimal(6,3);
DECLARE @CS decimal(2,1);
...
IF RAND() > 0.5
    ...
    IF RAND() < 0.5
	    ...
	    VALUES(NEWID(), 'team1', 'http://test/poi', GETDATE()+@I+@CS, 2, 404);
	    END
	ELSE
	    BEGIN
	    INSERT INTO leaderboard.dbo.LogMessages
	    (Id, TeamName, EndpointUri, CreatedDate, [Type], StatusCode)
	    VALUES(NEWID(), 'team1', 'http://test/poi', GETDATE()+@I+@CS, 2, 500);
	    END
    END
SET @Start = @Start+1;
END;

--Team 1, Challenge 1, Challenge Start, USER = 1
SET @Start = 200;
SET @End = 250;
SET @CS = 0.0;
SET @I = 0.0;
...
IF RAND() > 0.5
    ...
    IF RAND() < 0.5
	    ...
	    INSERT INTO leaderboard.dbo.LogMessages
	    (Id, TeamName, EndpointUri, CreatedDate, [Type], StatusCode)
	    VALUES(NEWID(), 'team1', 'http://test/user', GETDATE()+@I+@CS, 1, 404);
	    END
	ELSE
	    BEGIN
	    INSERT INTO leaderboard.dbo.LogMessages
	    (Id, TeamName, EndpointUri, CreatedDate, [Type], StatusCode)
	    VALUES(NEWID(), 'team1', 'http://test/user', GETDATE()+@I+@CS, 1, 500);
	    END
    END
SET @Start = @Start+1;
END;

--Team 1, Challenge 2, Challenge Start, USER = 1
SET @Start = 40;
SET @End = 50;
SET @CS = 0.5001;
SET @I = 0.0;
...
IF RAND() > 0.5
    ...
    IF RAND() < 0.5
	    ...
	    INSERT INTO leaderboard.dbo.LogMessages
	    (Id, TeamName, EndpointUri, CreatedDate, [Type], StatusCode)
	    VALUES(NEWID(), 'team2', 'http://test/user', GETDATE()+@I+@CS, 1, 404);
	    END
	ELSE
	    BEGIN
	    INSERT INTO leaderboard.dbo.LogMessages
	    (Id, TeamName, EndpointUri, CreatedDate, [Type], StatusCode)
	    VALUES(NEWID(), 'team2', 'http://test/user', GETDATE()+@I+@CS, 1, 500);
	    END
    END
SET @Start = @Start+1;
END;
...
IF RAND() > 0.5
    ...
    IF RAND() < 0.5
	    ...
	    INSERT INTO leaderboard.dbo.LogMessages
	    (Id, TeamName, EndpointUri, CreatedDate, [Type], StatusCode)
	    VALUES(NEWID(), 'team2', 'http://test/poi', GETDATE()+@I+@CS, 2, 404);
	    END
	ELSE
	    BEGIN
	    INSERT INTO leaderboard.dbo.LogMessages
